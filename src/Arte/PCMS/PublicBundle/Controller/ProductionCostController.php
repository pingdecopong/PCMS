...
use Arte\PCMS\PublicBundle\Form\TBProductionCostModel2;
use Arte\PCMS\AdminBundle\Form\TBProductionCostSearchType;
use Arte\PCMS\PublicBundle\Form\TBProductionCostType2;
...
//            ))
        ...
        $costs = $projectManager->getCostList($pageNo, $pageSize, $sortColumn['db_column_name'], $pageSortType, $searchParam, $this->getUser());
     ...
     *
     * @Route("/new", name="public_cost_new")
     * @Method({"GET", "POST"})
     * @Template()
     */
...
//        $formFactory = $this->get('form.factory');
        ...
        $formModel = new TBProductionCostModel2();
//        $formModel->setTBSystemUser($this->getUser());
        $formType = new TBProductionCostType2($this->get('security.context'));
...
//            $formModel->setTBSystemUser($this->getUser());
//            $form->get('TBProductionCost')->get('TBSystemUser')->setData($this->getUser());
                ...
                if($form->isValid())
                {
                    ...
                    }else if($form->get('Submit')->isClicked())
                    {
                        ...
                        try{
                            ...
                            $productionCost->setWorker($this->getUser());
                        ...
                        return $this->redirect($this->generateUrl('public_cost_show', array('id' => $cost->getId(), 'ret' => $returnParam)));
                    }
                }
            }else
            {
...
//            ->getForm();
     ...
     * TBProductionCost詳細
     *
     * @Route("/show/{id}", name="public_cost_show")
     * @Method("GET")
     * @Template()
     */
...
//        }
        ...
        if($entity->getWorker()->getId() != $this->getUser()->getId()){
     ...
     * TBProductionCost編集
     *
     * @Route("/edit/{id}", name="public_cost_edit")
     * @Method({"GET", "POST"})
     * @Template("ArtePCMSPublicBundle:ProductionCost:new.html.twig")
     */
...
//        $formType = new TBProductionCostType2();
        $formType = new TBProductionCostType2($this->get('security.context'));
     ...
     * TBProductionCost削除
     *
     * @Route("/delete/{id}", name="public_cost_delete")
     * @Method({"GET", "POST"})
     * @Template()
     */
    public function deleteAction($id)
    {
        ...
        if($request->isMethod('POST'))
        {
            ...
            if($form->get('Submit')->isClicked() && $form->isValid())
            {
                ...
                return $this->redirect($this->generateUrl('public_cost').'?'.urldecode($returnParam));
            }
        }
